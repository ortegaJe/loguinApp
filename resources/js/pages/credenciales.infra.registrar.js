class RenderDataSolicitudLoguinInfra {
  ...
  static async fetchSolicitudLoguinData(solicitudId) {
    ...
    try {
      ...
      if (
        !data.usuario ||
        !data.usuario[0] ||
        !data.loguin_infra_solicitud ||
        !data.loguin_infra_solicitud[0] ||
  ...
  static async showDataUserBlock(usuario, especialidadUsuario) {
    ...
    blockDataUser.querySelector("#modal-ticket-numero").innerHTML =
      `<span class="badge bg-${usuario.status_color}"><i class="${usuario.status_icon} me-1"></i>${usuario.ticket_id}</span>` ||
      "N/A";
  ...
  static async renderApplicationBlocks(loguinSolicitud) {
    ...
    Object.entries(loguinSolicitud).forEach(([key, value]) => {
      if (value === 1 && infraElements[key]) {
        ...
        infraSolicitudSpan.classList.add(
          "badge",
          "bg-primary-lighter",
          "infra"
        );
        infraSolicitudSpan.setAttribute("data-solicitud-key", `${key}`);
        infraSolicitudSpan.setAttribute(
          "data-solicitud-value",
          `${infraElements[key]}`
        );
  ...
  static async fetchAndRenderLoguins(solicitudId) {
    try {
      const response = await fetch(
        `/getLoguins/infraestructura/${solicitudId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
