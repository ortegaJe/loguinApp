class RenderDataSolicitudLoguin {
  ...
  static async hasLoguin(loguins) {
    ...
    const validLoguins = loguins.filter(
      (loguin) =>
        loguin.usuario_loguin !== null &&
        loguin.password_loguin !== null &&
        loguin.aplicacion_id !== 7 &&
        loguin.aplicacion_id !== 8
    );
    const validLoguinFilterMipres = loguins.filter(
      (loguin) => loguin.aplicacion_id === 7 && loguin.mipres !== null
    );
    const validLoguinFilterRuaf = loguins.filter(
    ...
    if (
      ...
      validLoguins.forEach((loguin) => {
        const table = this.createLoguinTable(
          loguin.aplicacion_perfil,
          loguin.usuario_loguin,
          loguin.password_loguin
        );
      ...
      validLoguinFilterMipres.forEach((loguin) => {
        const table = this.createLoguinMipresTable(
          loguin.aplicacion_perfil,
          loguin.mipres
        );
        loguinContent.appendChild(table);
  ...
  static async renderApplicationBlocks(loguinSolicitud) {
    ...
    if (corazaApps.length > 0) {
      ...
      corazaBlockDiv.setAttribute(
        "data-solicitud-id",
        `${loguinSolicitud[0].solicitud_id}`
      );
      ...
      corazaFormRowDiv.classList.add(
        "row",
        "row-cols-lg-auto",
        "g-3",
        "align-items-center"
    ...
    mipres.forEach((mipres) => {
      ...
      appProfileSpanMipres.setAttribute(
        "data-app-id",
        `${mipres.aplicacion_id}`
      );
      appProfileSpanMipres.setAttribute(
        "data-perfil-id",
        `${mipres.perfil_id}`
  ...
  static async handleSubmit(event) {
    ...
    if (!jQuery("#form-loguin").valid()) {
      ...
      console.log(
        "El formulario contiene campos que deben ser validados, no se enviará."
      );
      return;
    }
    ...
    appsContainers.forEach((container) => {
      ...
      const loguinUser =
        container.querySelector(".usuario-loguin")?.value || null;
      const loguinPassword =
        container.querySelector(".password-loguin")?.value || null;

      const mipresCheckbox = container.querySelector(
        'input[type="checkbox"][id="mipres"]'
      );
      const mipres = mipresCheckbox ? (mipresCheckbox.checked ? 1 : 0) : null;

      const ruafCheckbox = container.querySelector(
        'input[type="checkbox"][id="ruaf"]'
      );
        ...
        if (result.value) {
          try {
            ...
            if (!response.ok) {
              this.showToast(
                "Error...",
                `Error al guardar credenciales Loguin ${response.statusText}`,
                "error"
              );
              throw new Error(
                `Error en la respuesta del servidor: ${response.statusText} - ${response.status}`
              );
            }
            ...
            this.showToast(
              "Error...",
              `Error al guardar credenciales loguin ${error}`,
              "error"
            );
          }
  ...
  static init() {
    ...
    this.formLoguin.addEventListener("submit", (event) =>
